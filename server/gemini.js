...
const MAX_RETRY_COUNT = Number(process.env.GEMINI_MAX_RETRY_COUNT || 1);
...
사용자가 제공하는 화자의 구어체 STT(음성 인식) 원문 텍스트를 분석하여, 불필요한 감탄사와 반복을 제거하고 담백한 자서전 문체로 정제해야 한다.
입력된 약 2,500자의 구어체 원문을 약 500자 내외로 압축한다. 목표 압축률은 약 80%다.

[핵심 제약사항]
1. 문체 및 구성:
  - 본문은 신뢰성 있고 완성도 높은 문어체(~했다, ~였다)로 작성한다.
  - 화자의 시점을 보존한다. 원문이 1인칭이면 반드시 1인칭으로 쓴다.
  - 화자를 어머니, 아버지, 자녀 등 다른 인물로 바꾸지 않는다.
  - 원문에 나온 인물, 장소, 사건, 감정만 사용한다.
  - 원문에 없는 학교 중단, 밭일, 빨래, 헌신, 행복한 가정, 인물의 생각, 감사, 사랑, 가족관계 평가, 결말을 절대 만들지 않는다.
  - 원문에 없는 내용을 추측해야 할 경우 쓰지 말고 생략한다.
...
{
  "episode": {
    "title": "원문 핵심 사건을 담은 직관적인 제목 (15자 내외)",
      ...
      if (!response.ok) {
        ...
        if (!RETRYABLE_STATUS_CODES.has(response.status) || attempt === MAX_RETRY_COUNT) {
          ...
          if (response.status === 429) {
            message = "Gemini 무료 할당량을 초과했습니다. 잠시 후 다시 시도해주세요.";
          }
...
module.exports = { transcribeAudio, generateMemoir, generateEpisodeFromText, isGeminiConfigured };
